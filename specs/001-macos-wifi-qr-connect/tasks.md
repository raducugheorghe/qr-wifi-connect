...
- [X] T003 [P] Configure `tests/QrWifiConnect.Tests/QrWifiConnect.Tests.csproj` — add NuGet references: `xunit` 2.x, `NSubstitute` 5.x, `Microsoft.NET.Test.Sdk`; add project reference to `src/QrWifiConnect`
...
- [X] T013 Create `src/QrWifiConnect/AppShell.xaml` and `AppShell.xaml.cs` — define Shell routes: `//scanner`, `confirmation`, `connecting`, `result`, `permissiondenied`; register all five pages (depends on T007–T012 so ViewModels can be injected)
...
- [X] T031 [P] Create `tests/QrWifiConnect.Tests/Fakes/FakeWifiConnector.cs` and `FakeCameraPermissionService.cs` — configurable return values; no hardware dependency
